...
class mysql_backup
	...
	function _Mysqlbackup($host,$dbname, $uid, $pwd, $output, $structure_only)
		...
		$con=mysqli_connect($this->host, $this->user, $this->pass, $dbname);
		...
		for ($a=0;$a<$nt;$a++)
			...
			for ($b=0;$b<$nf;$b++)
				{
				$fn = mysqli_fetch_field_direct($res2, $b)->name;
				$ft = mysqli_fetch_field_direct($res2, $b)->type;
				$fs = mysqli_fetch_field_direct($res2, $b)->length;
				$ff = mysqli_fetch_field_direct($res2, $b)->flags;
				...
				for ($c=0;$c<$nr;$c++)
					...
					for ($d=0;$d<$nf;$d++)
						...
						if ($ina[$d]==true)
							...
							$sql.="\"".mysqli_real_escape_string($con, $data)."\"";
		...
		if ($this->structure_only==true)
			...
			$fname = $output;
			if (!mysqli_connect($host, $uid, $pwd, $dbname)) {
			...
			while ($row = mysqli_fetch_row($result)) {
				...
				$result2 = mysqli_query($con, "SHOW CREATE TABLE `".$row[0]."`");
	...
	function Restore($sep=";")
		...
		//---------------------------------------------------
		$link = mysqli_connect($this->host, $this->user, $this->pass, $this->db) or (die (mysqli_error()));
		//---------------------------------------------------
		...
		for ($i=0;$i<count($SQL)-1;$i++)
			{
				//echo $SQL[$i] . $sep . '<BR>';
				$link->query($SQL[$i] . $sep, MYSQLI_USE_RESULT) or (die (mysqli_error()));
			}
		//-----------------------------------------------
	}
	//---------------------------------------------------------
